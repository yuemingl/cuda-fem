...
g++ --std=c++11 fea_symbolic_nvrtc_sparse.cpp -o fea_symbolic_nvrtc_sparse -I $CUDA_PATH/include -L $CUDA_PATH/lib64 -lginac -lnvrtc -lcuda -Wl,-rpath,$CUDA_PATH/lib64
*/
int main()
{
  ...
  const char *opts[] = {"--gpu-architecture=compute_30",
                        "--define-macro=MESH_W=5000",
                        "--define-macro=MESH_H=5000",
...
// Generate input for execution, and create output buffers.
  ...
  CUdeviceptr dA, dRowA, dColA, dX, dY, dGIdx;
  ...
  CUDA_SAFE_CALL(cuMemAlloc(&dA, NE*(NNODE*NNODE)*sizeof(float)));
  CUDA_SAFE_CALL(cuMemAlloc(&dRowA, NE*(NNODE*NNODE)*sizeof(int)));
  CUDA_SAFE_CALL(cuMemAlloc(&dColA, NE*(NNODE*NNODE)*sizeof(int)));

  // Execute kernal.
  void *args[] = { &dA, &dRowA, &dColA, &dX, &dY, &dGIdx};
  CUevent start, stop;
  float elapsed = 0;
  ...
  CUDA_SAFE_CALL(cuMemcpyDtoH(A, dA, NE*(NNODE*NNODE)*sizeof(float)));
  CUDA_SAFE_CALL(cuMemcpyDtoH(rowA, dRowA, NE*(NNODE*NNODE)*sizeof(int)));
  CUDA_SAFE_CALL(cuMemcpyDtoH(colA, dColA, NE*(NNODE*NNODE)*sizeof(int)));
  ...
  CUDA_SAFE_CALL(cuMemFree(dRowA));
