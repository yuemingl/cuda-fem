//share memory + hard code symbol output + sparse matrix + atomicAdd
...
cudaError_t assembleWithCuda()
{
    ...
    float *A  = (float*)malloc( M*MAX_NEIGHBOR*sizeof(float) );
    int *gNbrNodeLen = (int*)malloc( M*sizeof(int) );
    int *gNbrNodeIdx = (int*)malloc( M*MAX_NEIGHBOR*sizeof(int) );
    ...
    tStart = clock();
    float *dA = NULL;
    cudaMalloc((void**)&dA, M*MAX_NEIGHBOR*sizeof(float));
    int *dNbrNodeLen = NULL;
    cudaMalloc((void**)&dNbrNodeLen, M*sizeof(int));
    int *dNbrNodeIdx = NULL;
    cudaMalloc((void**)&dNbrNodeIdx, M*MAX_NEIGHBOR*sizeof(int));
    float *dX = NULL;
    ...
    cudaMemcpy(dA, A, M*MAX_NEIGHBOR*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(dNbrNodeLen, gNbrNodeLen, M*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(dNbrNodeIdx, gNbrNodeIdx, M*MAX_NEIGHBOR*sizeof(int), cudaMemcpyHostToDevice);
    ...
    for(int i=0; i<((NE+BLOCK_Z-1)/BLOCK_Z)/65536; i++) {
      fea_kernel << <65535, dim_block >> >(dA, dNbrNodeLen, dNbrNodeIdx, dX, dY, dGIdx);
    }
    int rem_dim = ((NE+BLOCK_Z-1)/BLOCK_Z) % 65535;
    fea_kernel << <rem_dim, dim_block >> >(dA, dNbrNodeLen, dNbrNodeIdx, dX, dY, dGIdx);
    ...
    tStart = clock();
    cudaStatus = cudaMemcpy(A, dA, M*MAX_NEIGHBOR*sizeof(float), cudaMemcpyDeviceToHost);
    ...
    for(size_t i=0; i<16 && i<M; i++) {
      for(size_t j=0; j<gNbrNodeLen[i]; j++) {
        std::cout << "(" << i << "," << gNbrNodeIdx[i*MAX_NEIGHBOR+j] << ")" << " " << A[i*MAX_NEIGHBOR+j] << "    ";
      }
      cout << endl;
    }

    cudaFree(dA);
    cudaFree(dNbrNodeLen);
    cudaFree(dNbrNodeIdx);
    cudaFree(dX);
    cudaFree(dY);
    cudaFree(dGIdx);
...
// /usr/local/cuda-8.0/bin/nvcc --std=c++11 fea_test_sm_sym_sparse2.cu -o fea_test_sm_sym_sparse2
// /usr/local/cuda-9.1/bin/nvcc --std=c++11 fea_test_sm_sym_sparse2.cu -o fea_test_sm_sym_sparse2
int main()
{
    assembleWithCuda();
    cudaDeviceReset();
